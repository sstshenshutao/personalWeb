...
@Mapper @Component public interface UserSecurityMapper {
  ...
  @Select("SELECT * FROM `user_security` WHERE uid=#{uid}")
  UserSecurity select (int uid);

  @Select("SELECT * FROM `user_security`")
  List<UserSecurity> selectAll ();

  @Update(
    "UPDATE `user_security` SET uid=#{uid},username=#{username},email=#{email},question1=#{question1},answer1=#{answer1},question2=#{question2},answer2=#{answer2},handy_number=#{handy_number},password=#{password}"
      + " WHERE uid=#{uid}")
